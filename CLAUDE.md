...
- **一个项目只能有一个 astro dev**：astro 7 用 `.astro/` 下的锁文件保证这点，
  ...
  already running」。
- **inkbrush 子模块已 fork 到自己账号**：`packages/astro-inkbrush` 的远端
  （`.gitmodules` 的 url）现在是 **`YufengJin/astro-inkbrush`**（2026-09-07
  从 `ventusff/astro-inkbrush` fork 而来，公开，本机有写权限），子模块里
  `origin` 指这个 fork、`upstream` 指 ventusff。本机的改动在 `ws02-ai` 分支上，
  已推到 fork；壳仓库的指针因此可以正常指向这些提交（公开 CI 用 https 从 fork
  按 sha 取得，已验证能拉到）。**提交流程**：改完 inkbrush 先在子模块里
  `git -C packages/astro-inkbrush push origin ws02-ai`，再在壳仓库
  `git add packages/astro-inkbrush` 移动指针、提交、推送——两步都要，只移指针
  不推 fork，CI 拉不到那个 sha。要同步上游改动就从 `upstream` merge/rebase 到
  `ws02-ai`。私有站跑的始终是子模块工作区（pnpm workspace 按路径链接，不看
  指针）；公开构建不挂 CMS，用哪一版 inkbrush 都一样。
